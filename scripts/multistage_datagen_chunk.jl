...
chunk_id = parse(Int, ARGS[3])      # 0-indexed chunk ID
n_chunks = parse(Int, ARGS[4])      # total number of chunks

n_samples = 2000
N = 10

# Compute sample range for this chunk
chunk_size = ceil(Int, n_samples / n_chunks)
start_idx = chunk_id * chunk_size + 1
end_idx = min((chunk_id + 1) * chunk_size, n_samples)

println("Chunk $chunk_id/$n_chunks: samples $start_idx to $end_idx (material $mat_index, $n_stages stages)")
flush(stdout)

filepath = joinpath(@__DIR__, "..", "data", "dataset_material$(mat_index)_$(n_stages)stage_chunk$(chunk_id).csv")
...
# Generate SAME Sobol sequence as original (deterministic, same seed)
Random.seed!(1234)
...
open(filepath, "w") do io
    header = "sample_idx,P0,ndot,tads,alpha,beta,PI,Pl"
    for s in 1:n_stages
    ...
    for i in start_idx:end_idx
        ...
        if !failed && length(stage_results) == n_stages
            line = "$i,$P0,$ndot,$tads,$alpha,$beta,$PI,$Pl"
            ...
            println(io, line)
            flush(io)
            println("Sample $i / $end_idx done (purity=$(round(final_pur, digits=4)))")
            flush(stdout)
        end
    end
end
println("Chunk $chunk_id completed for material $mat_index ($n_stages stages).")
